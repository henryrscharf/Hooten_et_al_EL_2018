...
pdf(file = paste0("../fig/mountain_lion/", out_mcmc$time_finished, "_mtl_reDyn_mcmc.pdf"),
    width = 9)
## priors ----
mode_s <- out_mcmc$initial$sigsq_s; var_s <- mode_s^1.7
...
#     i_plot = i + 1
#   }
#   polygon(x = c(i_plot - x, i_plot + rev(x)), c(density$x, rev(density$x)),
#           col = "gray", lwd = 1 + 1.2 * as.numeric(min(CDF_shown[i], 1 - CDF_shown[i]) < 0.05))
# }
## plot separate ----
...
axis_points <- c(-2, -1, 0, 1, 2) * signif(X_scale_factor, 1) * 2
...
axis_points <- c(-2, -1, 0, 1, 2) * signif(W_scale_factor, 1) * 2
