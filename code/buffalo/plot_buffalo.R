## libraries ----
library(reDyn); library(smover)
# library(spatstat)
## device ----
pdf(file = paste0("../fig/buffalo/", out_mcmc$time_finished, "_buffalo_reDyn_mcmc.pdf"),
    width = 9)
## plot priors ----
...
if(!"g0" %in% out_mcmc$fixed){
  chains_df <- cbind(chains_df, t(out_mcmc$chains$g0))
  colnames(chains_df)[ncol(chains_df)] <- "g0"
}
...
text(x =  (1 + nrow(out_mcmc$chains$beta)) / 2, y = par()$usr[4] + 0.05 * diff(par()$usr[3:4]), labels = "behavior", font = 2, xpd = T)
